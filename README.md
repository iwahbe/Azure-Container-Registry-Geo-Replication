...
This repo is a boilerplate showing how to create a Pulumi component provider written in Go. You can search-replace `registrygeoreplication` with the name of your desired provider as a starting point for creating a component provider for your component resources.
...
The `registrygeoreplication` provider's plugin binary must be named `pulumi-resource-registrygeoreplication` (in the format `pulumi-resource-<provider>`).
...
```json
"resources": {
    "registrygeoreplication:index:StaticPage": {
        "isComponent": true,
        "inputProperties": {
            "indexContent": {
                "type": "string",
...
The provider makes this component resource available in the `construct` function in `provider/pkg/provider/provider.go`. When `construct` is called and the `typ` argument is `registrygeoreplication:index:StaticPage`, we create an instance of the `StaticPage` component resource and return its `URN` and state.
